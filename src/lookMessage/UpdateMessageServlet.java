	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (phone.length()==0||email.length()==0){
			...
			try{
					...
					String sql1="Update myuser set edu= '"+edu+"',work= '"+work+"',phone='"+phone+"',email= '"+email+"' where userName= '"+userName+"'";
					...
					stmt.close();
					con.close();
				out.print("<script language='javascript'> alert('修改成功!');self.location='./lookManager/lookMessage.jsp';</script>");
			}catch(Exception e){
				e.printStackTrace();
			}
		}
